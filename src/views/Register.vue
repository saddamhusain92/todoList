<template>
  <div class="max-w-screen-sm mx-auto px-4 py-10">
    <!-- Error Handling -->
    <div v-if="errorAlert" class="mb-10 p-4 rounded-md bg-light-grey shadow-lg">
      <p class="text-red-500">{{ errorAlert }}</p>
    </div>
    ...
    const register = async () => {
      if (password.value === confirmPassword.value) {
        try {
          ...
          errorAlert.value = error.message;
          setTimeout(() => {
      ...
      errorAlert.value = "Error: Passwords do not match";
      setTimeout(() => {
        errorAlert.value = null;
      }, 5000);
    };
    return { email, password, confirmPassword, errorAlert, register };
  },
};
</script>